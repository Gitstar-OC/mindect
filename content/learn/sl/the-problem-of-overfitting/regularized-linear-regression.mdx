...
description: In this part, we'll figure out how to get gradient descent to work with regularized linear regression.
---
...
```math
\frac{dJ}{dw_j} = \frac{1}{m} \sum (h_\theta(x^{(i)}) - y^{(i)}) x_j^{(i)} + \frac{\lambda}{m} w_j
```
...
If we simplify, then we're saying that w_j is updated as w_j times `1 - Alpha \times Lambda / m`, minus Alpha times the other term.
