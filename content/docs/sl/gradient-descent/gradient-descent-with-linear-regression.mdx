...
```math
\frac{\partial}{\partial w} J(w) = \frac{1}{m} \sum_{i=1}^{m} (h(x^{(i)}) - y^{(i)}) x^{(i)}
```
...
```math
\frac{\partial}{\partial b} J(w) = \frac{1}{m} \sum_{i=1}^{m} (h(x^{(i)}) - y^{(i)})
```
...
![GDLR9](/SL/GradientDescent/GDLR9.png)
