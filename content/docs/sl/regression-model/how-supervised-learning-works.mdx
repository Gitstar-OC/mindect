...
description: In this section you will learn how supervised learning works.
label: <kbd> Beginner <kbd>
---


## Introduction
Let's look in this part at the process of how supervised learning works. 

[Supervised learning algorithm](/docs/supervised-learning#supervised-learning-algorithms) will input a dataset and then what exactly does it do and what does it output? You will find out. 


Recall that a `training set` in [supervised learning](/docs/supervised-learning) includes both the `input features`, such as **the size of the house** and also the `output targets`, such as **the price of the house**. The output targets are the right answers to the model we'll learn from. 
...
Now, when we design a learning algorithm, a key question is, **How are we going to represent the function f? Or in other words, what is the math formula we're going to use to compute f?** For now, let's stick with **f** being a _straight_ line. You're function can be written as `f_w,b of x equals, I'm going to use w times x plus b`.
