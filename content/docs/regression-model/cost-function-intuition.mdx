...
description: How the cost function can be used to find the best parameters for your model ? 
---
...
In this part of intuition, you'll walk through one example to see. You want to fit a straight line to the training data, so you have this model, `fw, b of x is w times x, plus b`. Here, the model's parameters are `w`, and `b`.

## What Cost Function J does ?
...
## Conclusion
Now in this part, we worked through our example with a simplified problem using `only w`. In the next part, you'll visualize what the cost function looks like for the full version of linear regression using `both w and b`. You see some `cool 3D plots`.