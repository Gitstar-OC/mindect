...
const ListItem: React.FC<ListItemProps> = ({ title, description, icon: Icon, className }) => {
  if (!Icon) {
    console.error(`Icon component is undefined for title: ${title}`);
    return null;
  }
      ...
      <NavigationMenuLink asChild>
        ...
        >
          <div className="flex items-center gap-2">
            <div
              className={cn(
                `flex h-8 w-8 items-center justify-center rounded-md bg-neutral-800 text-white transition-colors group-hover:bg-gradient-to-br`,
                className
              )}
            >
              <Icon className="h-4 w-4" />
